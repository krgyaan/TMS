...
interface TenderPaymentDetailsResponse {
    id: number;
    tenderNo: string;
    tenderName: string;
...
export const useEmdTenderFeeDetails = (tenderId: number | null) => {
    return useQuery<TenderPaymentDetailsResponse>({
        queryKey: paymentRequestsKey.tenderPaymentDetails(tenderId ?? 0),
        queryFn: async () => {
            if (!tenderId) return null;
            return tenderInfosService.getPaymentDetails(tenderId);
        },
        enabled: !!tenderId,
        staleTime: Infinity,
    });
