...
import { SubmitQuerySection } from './components/SubmitQueryView';
...
export default function SubmitQueryShowPage() {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const queryId = id ? parseInt(id, 10) : 0;

    const { steps: tenderSteps, tender } = useTenderStepStatuses(null, { submitQueryId: queryId });
    const tenderId = tender?.id || null;

    const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(["submit-query"]));
    ...
    const renderSectionContent = (stepId: string) => {
        switch (stepId) {
            ...
            case "submit-query":   return <SubmitQuerySection queryId={queryId} />;
    ...
    return (
        <ShowPageLayout
            steps={tenderSteps.filter(s => ["tender-details", "physical-docs", "submit-query", "rfq", "emd-fees", "checklist", "costing", "bid", "tq-management", "ra-management", "result"].includes(s.id))}
