...
export const DemandDraftView = ({ instruments }: DemandDraftViewProps) => {
    if (!instruments || instruments.length === 0) {
        return (
            <div className="text-sm text-muted-foreground py-4">
                No Demand Draft instruments found.
            </div>
        );
    }
                        ...
                        <TableBody>
                            ...
                            {hasValue(instrument.issueDate) && (
                                <TableRow className="hover:bg-muted/30">
                                    ...
                                    {hasValue(instrument.details?.ddDate) && (
                                        <>
                                            <TableCell className="text-sm font-medium text-muted-foreground">DD Date</TableCell>
                                            <TableCell className="text-sm">{formatDate(instrument.details?.ddDate)}</TableCell>
                                        </>
                                    )}
                                </TableRow>
                            )}
                            {hasValue(instrument.details?.ddNo) && (
                                <TableRow className="hover:bg-muted/30">
                                    <TableCell className="text-sm font-medium text-muted-foreground">DD Number</TableCell>
                                    <TableCell colSpan={3} className="text-sm">{instrument.details?.ddNo}</TableCell>
                                </TableRow>
                            )}
                            {hasValue(instrument.details?.reqNo) && (
                                <TableRow className="hover:bg-muted/30">
                                    <TableCell className="text-sm font-medium text-muted-foreground">Request Number</TableCell>
                                    <TableCell colSpan={3} className="text-sm">{instrument.details?.reqNo}</TableCell>
                                </TableRow>
                            )}
                            {hasValue(instrument.details?.ddNeeds) && (
                                <TableRow className="hover:bg-muted/30">
                                    <TableCell className="text-sm font-medium text-muted-foreground">DD Needs</TableCell>
                                    <TableCell colSpan={3} className="text-sm">{instrument.details?.ddNeeds}</TableCell>
                                </TableRow>
                            )}
                            {hasValue(instrument.details?.ddPurpose) && (
                                <TableRow className="hover:bg-muted/30">
                                    <TableCell className="text-sm font-medium text-muted-foreground">DD Purpose</TableCell>
                                    <TableCell colSpan={3} className="text-sm">{instrument.details?.ddPurpose}</TableCell>
                                </TableRow>
                            )}
                            {hasValue(instrument.details?.ddRemarks) && (
                                <TableRow className="hover:bg-muted/30">
                                    <TableCell className="text-sm font-medium text-muted-foreground">Remarks</TableCell>
                                    <TableCell colSpan={3} className="text-sm">{instrument.details?.ddRemarks}</TableCell>
                                </TableRow>
                            )}
                                ...
                                <TableRow className="hover:bg-muted/30">
                                    ...
                                    <TableCell colSpan={3} className="text-sm">
                                        <div className="flex flex-wrap gap-4">
                                            {instrument.generatedPdf && renderFileLink(instrument.generatedPdf, "Generated DD")}
