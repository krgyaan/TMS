...
interface ChequeDetails {
    ...
    chequeNeeds?: string | null;
    chequeReason?: string | null;
}
...
export const ChequeView = ({ instruments }: ChequeViewProps) => {
    if (!instruments || instruments.length === 0) {
        return (
            <div className="text-sm text-muted-foreground py-4">
                No Cheque instruments found.
            </div>
        );
    }
                        ...
                        <TableBody>
                            ...
                            {hasValue(instrument.issueDate) && (
                                <TableRow className="hover:bg-muted/30">
                                    ...
                                    {hasValue(instrument.details?.chequeDate) && (
                                        <>
                                            <TableCell className="text-sm font-medium text-muted-foreground">Cheque Date</TableCell>
                                            <TableCell className="text-sm">{formatDate(instrument.details?.chequeDate)}</TableCell>
                                        </>
                                    )}
                                </TableRow>
                            )}
                            {hasValue(instrument.details?.chequeNo) && (
                                <TableRow className="hover:bg-muted/30">
                                    <TableCell className="text-sm font-medium text-muted-foreground">Cheque Number</TableCell>
                                    <TableCell colSpan={3} className="text-sm">{instrument.details?.chequeNo}</TableCell>
                                </TableRow>
                            )}
                            ...
                            {hasValue(instrument.details?.chequeNeeds) && (
                                <TableRow className="hover:bg-muted/30">
                                    <TableCell className="text-sm font-medium text-muted-foreground">Cheque Needs</TableCell>
                                    <TableCell colSpan={3} className="text-sm">{instrument.details?.chequeNeeds}</TableCell>
                                </TableRow>
                            )}
                            {hasValue(instrument.details?.chequeReason) && (
                                <TableRow className="hover:bg-muted/30">
                                    <TableCell className="text-sm font-medium text-muted-foreground">Cheque Reason</TableCell>
                                    <TableCell colSpan={3} className="text-sm">{instrument.details?.chequeReason}</TableCell>
                                </TableRow>
                            )}
                                ...
                                <TableRow className="hover:bg-muted/30">
                                    ...
                                    <TableCell colSpan={3} className="text-sm">
                                        <div className="flex flex-wrap gap-4">
                                            {instrument.generatedPdf && renderFileLink(instrument.generatedPdf, "Generated Cheque")}
