...
type DocumentSubmittedModalProps = {
    open: boolean;
...
export const DocumentSubmittedModal = ({ open, onOpenChange, documentSubmitted, onSuccess }: DocumentSubmittedModalProps) => {
    ...
    useEffect(() => {
        ...
        createDocumentSubmitted.reset?.();
        updateDocumentSubmitted.reset?.();
    }, [open, isEdit, documentSubmitted, form]);
    ...
    const handleSubmit = async (values: DocumentSubmittedFormValues) => {
        try {
            if (isEdit && documentSubmitted) {
                await updateDocumentSubmitted.mutateAsync({
                    id: documentSubmitted.id,
                    data: values,
                });
            } else {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[420px] p-0 gap-0 overflow-hidden">
                ...
                <DialogHeader className="px-6 pt-6 pb-4">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 space-y-1">
                            <DialogTitle className="text-lg">{isEdit ? "Edit Document Type" : "Create Document Type"}</DialogTitle>
                            <DialogDescription className="text-sm">{isEdit ? "Update document type details" : "Add a new document type to the system"}</DialogDescription>
                        </div>

                        {isEdit && documentSubmitted && (
                ...
                <div className="px-6 py-5">
                    <Form {...form}>
                        <form id="document-submitted-form" onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
                            {/* Name */}
                            <FormField
                                    ...
                                    <FormItem>
                                        ...
                                        >
                                            <div className="space-y-0.5">
                                                ...
                                                <p className="text-xs text-muted-foreground">{field.value ? "Document type is enabled" : "Document type is disabled"}</p>
                                            </div>
                                            <FormControl>
                ...
                <DialogFooter className="px-6 py-4 bg-muted/30">
                    <div className="flex justify-end gap-2 w-full">
                        ...
                        <Button type="submit" form="document-submitted-form" disabled={saving} className="min-w-[110px]">
                            {saving ? (
                                <>
