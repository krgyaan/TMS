  ...
  return (
    <div className="flex items-center gap-2">
      <input
        ...
        onChange={handleFile}
      />
      <Button type="button" variant="outline" size="sm" className="h-8" disabled={uploading} onClick={() => inputRef.current?.click()}>
...
const VwoClosurePage = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const woId = Number(id);

  const [wo, setWo] = useState<VwoClosureData | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError("Failed to load vendor work order closure data");
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  const savePayments = async () => {
    ...
    setSaveMsg(null);
    try {
      await vendorWorkOrderApi.bulkCreatePaymentRequests(woId, valid);
      setPaymentRows([]);
  ...
  const saveInvoices = async () => {
    ...
    setSaveMsg(null);
    try {
      await vendorWorkOrderApi.bulkCreatePurchaseInvoices(woId, valid);
      setInvoiceRows([]);
  ...
  if (error || !wo) {
    return (
      <div className="flex justify-center py-12">
        <p className="text-destructive">{error || "Vendor Work Order not found"}</p>
      </div>
    );
  }
  ...
  return (
    <Card>
      <CardHeader className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">VWO Closure — {wo.woNumber}</h1>
        <Button variant="outline" onClick={() => navigate(paths.accounts.vendorWorkOrders)}>
          Back to VWOs
        </Button>
      </CardHeader>
                    ...
                    {invoiceRows.map((row, i) => (
                      <TableRow key={i} className="hover:bg-muted/30">
                        ...
                        <TableCell>
                          <Input
                            className="h-8 w-44"
                            placeholder={wo.sellerName || "Party name"}
                            value={row.partyName}
                        ...
                        <TableCell>
                          <FileUploadCell value={row.invoiceFile} onChange={(paths) => updateInvoiceRow(i, "invoiceFile", paths)} />
                        </TableCell>
                        <TableCell>
                          <Button
                            type="button"
          ...
          {wo.paymentRequests.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {wo.paymentRequests.map((pr) => {
                    const cfg = { label: pr.status, variant: "outline" as const };
                    return (
