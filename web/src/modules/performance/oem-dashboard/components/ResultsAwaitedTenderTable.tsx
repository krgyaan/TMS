...
interface ResultsAwaitedTenderTableProps {
    params: OemPerformanceParams | null;
}

export default function ResultsAwaitedTenderTable({ params }: ResultsAwaitedTenderTableProps) {
    const navigate = useNavigate();
    const { data, isLoading } = useOemPerformance(params);
    const [isOpen, setIsOpen] = useState(true);

    const tenders = useMemo(() => data?.tendersByKpi.tenderResultsAwaited ?? [], [data]);
                ...
                <CardHeader className="pb-4">
                    <CollapsibleTrigger asChild>
                        <div className="flex cursor-pointer items-center justify-between gap-2 transition-opacity hover:opacity-80">
                            <div>
                                <CardTitle className="text-base font-semibold">Tender Results Awaited</CardTitle>
                                <CardDescription>Tenders awaiting final results.</CardDescription>
                            </div>
