...
interface LostTenderTableProps {
    params: OemPerformanceParams | null;
}

export default function LostTenderTable({ params }: LostTenderTableProps) {
    const navigate = useNavigate();
    const { data, isLoading } = useOemPerformance(params);
    const [isOpen, setIsOpen] = useState(true);

    const tenders = useMemo(() => data?.tendersByKpi.tendersLost ?? [], [data]);
                ...
                <CardHeader className="pb-4">
                    <CollapsibleTrigger asChild>
                        <div className="flex cursor-pointer items-center justify-between gap-2 transition-opacity hover:opacity-80">
                            <div>
                                <CardTitle className="text-base font-semibold">Tenders Lost</CardTitle>
                                <CardDescription>Tenders that were lost.</CardDescription>
                            </div>
