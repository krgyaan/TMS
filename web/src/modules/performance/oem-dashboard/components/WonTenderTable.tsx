...
interface WonTenderTableProps {
    params: OemPerformanceParams | null;
}

export default function WonTenderTable({ params }: WonTenderTableProps) {
    const navigate = useNavigate();
    const { data, isLoading } = useOemPerformance(params);
    const [isOpen, setIsOpen] = useState(true);

    const tenders = useMemo(() => data?.tendersByKpi.tendersWon ?? [], [data]);
                ...
                <CardHeader className="pb-4">
                    <CollapsibleTrigger asChild>
                        <div className="flex cursor-pointer items-center justify-between gap-2 transition-opacity hover:opacity-80">
                            <div>
                                <CardTitle className="text-base font-semibold">Tenders Won</CardTitle>
                                <CardDescription>Tenders that were won.</CardDescription>
                            </div>
