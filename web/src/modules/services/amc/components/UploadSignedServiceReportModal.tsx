...
interface UploadSignedServiceReportModalProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    amcId: number | null;
}

export function UploadSignedServiceReportModal({
    open,
    onOpenChange,
    amcId,
}: UploadSignedServiceReportModalProps) {
    const { data: amc } = useAmc(amcId ?? 0);
    const uploadFile = useAmcFileUpload();
    const [file, setFile] = useState<File | null>(null);

    const existing = amc?.signedServiceReportPath;
    ...
    const handleUpload = async () => {
        ...
        try {
            await uploadFile.mutateAsync({ id: amcId, field: "signed-service-report", file });
            handleClose();
        } catch {
            // handled by hook
        }
            ...
            <DialogContent className="sm:max-w-[480px]">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        <FileSignature className="h-5 w-5" /> Upload Signed Service Report
                    </DialogTitle>
                    <DialogDescription>
                        Upload the signed service report for this AMC. It replaces any previously uploaded signed report.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="signed-report">Select signed service report file</Label>
                        <input
                            id="signed-report"
                            type="file"
