...
export function QuoteSubmissionModal({
    ...
    const handleConfirm = async () => {
        ...
        try {
            if (docs.length > 0) {
                await leadsQuotationService.uploadDocs(quote.id, docs);
            }

            const finalDocs = docs.join(',');
    ...
    const canSubmit = quoteSubmissionDatetime && contacts.some(c => c.name.trim());
