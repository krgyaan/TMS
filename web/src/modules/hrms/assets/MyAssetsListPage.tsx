...
import { Box, Calendar, FileText, Hash, Info, Key, Laptop, Monitor, PackageOpen, Settings, Smartphone, Tag, UserRound } from "lucide-react";
import { useState } from "react";
import { formatAssetDate } from "./helpers/asset.mappers";

export default function MyAssetsListPage() {
            ...
            <Card key={asset.id} className="overflow-hidden group hover:-translate-y-1 transition-transform duration-300 hover:shadow-md border-muted">
              ...
              <CardContent className="p-5">
                ...
                <div className="text-xs text-muted-foreground flex flex-col gap-1.5 mt-3">
                  ...
                  <span className="flex items-center gap-2"><Calendar className="h-3 w-3" /> Assigned: {formatAssetDate(asset.assignedDate)}</span>
                </div>
              </CardContent>
            ...
            <>
              <DialogHeader>
                <div className="flex items-center gap-4 mb-2">
                  ...
                  <div>
                    <DialogTitle className="text-xl">{selectedAsset.brand} {selectedAsset.model}</DialogTitle>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 py-4 mt-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div><span className="text-muted-foreground block mb-1">Assigned Date</span> <span className="font-medium">{formatAssetDate(selectedAsset.assignedDate)}</span></div>
                    <div><span className="text-muted-foreground block mb-1">Expected Return</span> <span className="font-medium">{formatAssetDate(selectedAsset.expectedReturnDate)}</span></div>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      {selectedAsset.licenseKey && (
                        <div className="col-span-2">
                          <span className="text-muted-foreground mb-1 flex items-center gap-1"><Key className="h-3 w-3" /> License Key</span>
                          <span className="font-medium font-mono text-xs">{selectedAsset.licenseKey}</span>
                        </div>
                      )}
                      {selectedAsset.warrantyFrom && (
                        <div><span className="text-muted-foreground block mb-1">Warranty From</span> <span className="font-medium">{formatAssetDate(selectedAsset.warrantyFrom)}</span></div>
                      )}
                      {selectedAsset.warrantyTo && (
                        <div><span className="text-muted-foreground block mb-1">Warranty Until</span> <span className="font-medium text-amber-600">{formatAssetDate(selectedAsset.warrantyTo)}</span></div>
                      )}
                    </div>
                  </div>
                )}
                      ...
                      {selectedAsset.assignmentFormUrl && (
                        <Button variant="outline" size="sm" asChild>
                          <a href={`http://localhost:3000/${selectedAsset.assignmentFormUrl.replace("./", "")}`} target="_blank" rel="noreferrer">
                            <FileText className="h-4 w-4 mr-2" /> Assignment Form
                          </a>
                        </Button>
                      )}
                      {selectedAsset.warrantyCardUrl && (
                        <Button variant="outline" size="sm" asChild>
                          <a href={`http://localhost:3000/${selectedAsset.warrantyCardUrl.replace("./", "")}`} target="_blank" rel="noreferrer">
                            <FileText className="h-4 w-4 mr-2" /> Warranty Document
                          </a>
                        </Button>
                      )}
                    </div>
