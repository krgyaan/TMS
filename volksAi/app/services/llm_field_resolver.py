...
class LLMFieldResolver:
    ...
    def _call_gemini_v2(
        ...
        # Cap retries to 2 attempts with a short 2s backoff to stay well within budget
        max_retries = 2
        for attempt in range(max_retries):
            try:
                ...
                if ("429" in err_str or "RESOURCE_EXHAUSTED" in err_str) and attempt < max_retries - 1:
                    logger.warning(
                        "[LLM_FALLBACK] Rate limited (429). Waiting 2s before retry (attempt %d/%d)...",
    ...
    def _call_anthropic(self, system_instruction: str, user_prompt: str, timeout: float = 25.0) -> str:
    ...
    def _call_openai_compatible(self, system_prompt: str, user_prompt: str, timeout: float = 25.0) -> str:
    ...
    def resolve(
        ...
        while attempt < max_retries:
            ...
            try:
                if self.provider == "anthropic":
                    try:
                        raw_text = self._call_anthropic(system_instruction, user_prompt, timeout=25.0)
                        ...
                        if groq_key and groq_key.startswith("gsk_") and "placeholder" not in groq_key.lower():
                            ...
                            raw_text = self._call_openai_compatible(system_instruction, user_prompt, timeout=25.0)
                        else:
                            raise anthropic_err
                elif self.provider == "gemini":
                    try:
