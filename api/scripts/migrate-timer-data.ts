...
// import { stepInstances, timerEvents, appSettings, businessCalendar } from '../src/db/schemas/workflow/workflows.schema';
...
//             .select({
//                 id: steps.id,
//                 templateId: steps.workflowTemplateId,
...
//             .select({
//                 id: instances.id,
//                 entityType: instances.entityType,
//                 entityId: instances.entityId,
//             })
//             .from(instances);
...
//           status, timer_status, assigned_to_user_id,
...
//         const [r1] = await this.pgDb.select({ maxId: sql`COALESCE(MAX(id), 0)` }).from(wfInstances);
//         const [r2] = await this.pgDb.select({ maxId: sql`COALESCE(MAX(id), 0)` }).from(wfStepInstances);
//         const [r3] = await this.pgDb.select({ maxId: sql`COALESCE(MAX(id), 0)` }).from(wfTimerEvents);

//         this.nextWorkflowInstanceId = r1.maxId + 1;
//         this.nextStepInstanceId = r2.maxId + 1;
//         this.nextTimerEventId = r3.maxId + 1;
//     }
