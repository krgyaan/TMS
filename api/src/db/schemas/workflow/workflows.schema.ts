import { pgTable, varchar, text, timestamp, jsonb, integer, boolean, pgEnum, index, bigserial } from "drizzle-orm/pg-core";
import { relations } from "drizzle-orm";
import { users } from "../auth/users.schema";

export const entityTypeEnum = pgEnum("entity_type", ["TENDER", "COURIER", "EMD", "SERVICE", "OPERATION"]);

export const timerStatusEnum = pgEnum("timer_status", ["NOT_STARTED", "RUNNING", "PAUSED", "COMPLETED", "OVERDUE", "SKIPPED", "CANCELLED"]);

export const stepStatusEnum = pgEnum("step_status", ["PENDING", "IN_PROGRESS", "COMPLETED", "SKIPPED", "REJECTED", "ON_HOLD"]);

/**
 * STEP INSTANCES
 * This is the main timer table (like Laravel's timer_trackers)
 */
export const stepInstances = pgTable(
    "step_instances",
    {
...
export const businessCalendar = pgTable(
    ...
    table => [{ dateIdx: index("business_calendar_date_idx").on(table.date) }, { isHolidayIdx: index("business_calendar_holiday_idx").on(table.isHoliday) }]
);
...
export const timer = {
    ...
    timerName: stepInstances.stepKey,
    userId: stepInstances.assignedToUserId,
    startTime: stepInstances.actualStartAt,
    endTime: stepInstances.actualEndAt,
