...
export class AuthController {
    ...
    @Get("google/callback")
    @Public()
    async googleCallback(@Query() query: Record<string, unknown>, @Res() res: Response) {
        ...
        try {
            ...
            redirect.searchParams.set("user", Buffer.from(JSON.stringify(session.user)).toString("base64url"));
