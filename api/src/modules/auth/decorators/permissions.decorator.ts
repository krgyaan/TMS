...
import { PERMISSIONS_KEY, PERMISSION_MODE_KEY, type PermissionRequirement } from "@/modules/auth/guards/permission.guard";
...
export const RequireAnyPermission = (...permissions: PermissionRequirement[]) =>
    applyDecorators(SetMetadata(PERMISSIONS_KEY, permissions), SetMetadata(PERMISSION_MODE_KEY, "any"));

// Convenience decorators for common actions
export const CanCreate = (module: string) => RequirePermissions({ module, action: "create" });

export const CanRead = (module: string) => RequirePermissions({ module, action: "read" });

export const CanUpdate = (module: string) => RequirePermissions({ module, action: "update" });
