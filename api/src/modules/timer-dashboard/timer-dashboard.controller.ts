...
export class TimerDashboardController {
    ...
    @Get("expiring-soon")
    async expiringSoon(@Query("hours") hours?: string, @Query("includeOverdue") includeOverdue?: string) {
        const withinHours = hours ? Number.parseInt(hours, 10) : 12;
        if (Number.isNaN(withinHours) || withinHours <= 0) {
            throw new BadRequestException('"hours" must be a positive number');
        }
        const showOverdue = includeOverdue === "true";
        return this.timerDashboardService.getExpiringSoon(withinHours, showOverdue);
    }

    @Get("search")
    async search(@Query("by") by: string, @Query("value") value: string) {
        if (!by || !value) {
    ...
    @Post("stop")
    async stop(@Body() body: { entityType: string; entityId: number; stage: string }) {
