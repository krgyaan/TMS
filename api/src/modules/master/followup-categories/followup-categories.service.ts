...
export class FollowupCategoriesService {
    ...
    async update(
        id: number,
        data: Partial<NewFollowupCategory>,
    ...
    async search(query: string): Promise<FollowupCategory[]> {
        ...
        return this.db
            .select()
            .from(followupCategories)
            .where(like(followupCategories.name, searchPattern));
    }
}
