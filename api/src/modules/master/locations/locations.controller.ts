...
const CreateLocationSchema = z.object({
    ...
    state: z.string().max(100, "State cannot be longer than 100 characters").optional().nullable(),
    region: z.string().max(100, "Region cannot be longer than 100 characters").optional().nullable(),
    acronym: z.string().max(20, "Acronym cannot be longer than 20 characters").optional().nullable(),
...
export class LocationsController {
    ...
    @Get("list-true")
    async listTrue() {
        return this.locationsService.findAllTrue();
    }
    ...
    async update(@Param("id", ParseIntPipe) id: number, @Body() body: unknown) {
        ...
        const payload: UpdateLocationDto = {
            ...parsed,
            name: parsed.name?.trim(),
            acronym: parsed.acronym === undefined ? parsed.acronym : parsed.acronym?.trim() || null,
            state: parsed.state === undefined ? parsed.state : parsed.state?.trim() || null,
            region: parsed.region === undefined ? parsed.region : parsed.region?.trim() || null,
        };
