...
export class TrainingController {
    ...
    async initUpload(@Body() body: { fileSize: string; originalName: string; totalChunks?: string }) {
        const totalChunks = body.totalChunks ? Number(body.totalChunks) : undefined;
        return this.uploadService.init({
            fileSize: Number(body.fileSize),
            originalName: body.originalName,
            totalChunks: Number.isFinite(totalChunks) && totalChunks! > 0 ? totalChunks : undefined,
        });
    }

    @Post("upload/chunk/:uploadId")
    async uploadChunk(@Param("uploadId") uploadId: string, @Headers("x-chunk-index") chunkIndex: string, @Req() req: Request) {
    ...
    async create(
        ...
        const video = await this.service.create(body.title, body.description, body.category, threshold, file, user.sub);
    ...
    @Public()
    @Get(":id/stream")
    async stream(@Param("id", ParseIntPipe) id: number, @Headers("range") rangeHeader: string, @Res() res: any) {
    ...
    @Post("assignments")
    async assign(@Body() body: { videoId: number; userIds: number[] }, @CurrentUser() user: any) {
    ...
    @Post("progress")
    async logProgress(@Body() body: { videoId: number; lastPositionSecs: number; totalWatchSecs: number; completionPct: number }, @CurrentUser() user: any) {
        return this.service.logProgress(body.videoId, user.sub, body.lastPositionSecs, body.totalWatchSecs, body.completionPct);
    }

    @Post("videos/:id/reactions")
    async addReaction(@Param("id", ParseIntPipe) id: number, @Body() body: { reaction: string }, @CurrentUser() user: any) {
        return this.service.addReaction(id, user.sub, body.reaction);
    }

    @Delete("videos/:id/reactions")
    async removeReaction(@Param("id", ParseIntPipe) id: number, @Body() body: { reaction: string }, @CurrentUser() user: any) {
        return this.service.removeReaction(id, user.sub, body.reaction);
    }

    @Get("videos/:id/reactions")
    async getReactions(@Param("id", ParseIntPipe) id: number, @CurrentUser() user: any) {
        return this.service.getReactions(id, user.sub);
    }

    @Post("videos/:id/comments")
    async addComment(@Param("id", ParseIntPipe) id: number, @Body() body: { body: string; parentCommentId?: number }, @CurrentUser() user: any) {
