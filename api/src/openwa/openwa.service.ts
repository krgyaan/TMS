import { Inject, Injectable, Logger, OnModuleInit } from '@nestjs/common';
import { ConfigType } from '@nestjs/config';
import { OpenWAApiError, OpenWAClient, OpenWARateLimitError, OpenWAServiceUnavailableError } from '@rmyndharis/openwa';
import { openwaConfig } from '../config/openwa.config';

@Injectable()
export class OpenwaService implements OnModuleInit {
  private readonly logger = new Logger(OpenwaService.name);
  private readonly client: OpenWAClient;
  private readonly sessionName: string;
  private readonly maxRetries = 3;
  private readonly baseRetryDelay = 1000;

  constructor(
    @Inject(openwaConfig.KEY)
    private readonly config: ConfigType<typeof openwaConfig>,
  ) {
    this.sessionName = this.config.OPENWA_SESSION_NAME;
    this.client = new OpenWAClient({
      baseUrl: this.config.OPENWA_BASE_URL,
      apiKey: this.config.OPENWA_API_KEY,
    });
  }

  async onModuleInit(): Promise<void> {
    try {
      await this.client.sessions.start(this.sessionName);
      this.logger.log(`OpenWA session "${this.sessionName}" started`);
    } catch (error) {
      if (
        error instanceof OpenWAApiError &&
        error.status === 400 &&
        error.body?.code === 'SESSION_ALREADY_STARTED'
      ) {
        this.logger.log(`Session "${this.sessionName}" already running`);
      } else {
        this.logger.error('Failed to start OpenWA session', error);
        throw error;
      }
    }
  }

  // ---- Public API ----

  /** Send a text message (individual or group) */
  async sendText(
    chatId: string,
    text: string,
    mentions?: string[],
  ): Promise<{ messageId: string }> {
    return this.withRetry(() =>
      this.client.messages.sendText(this.sessionName, { chatId, text, mentions }),
    );
  }

  /** Send a document (PDF, etc.) via URL or base64 */
  async sendDocument(
    chatId: string,
    options: {
      url?: string;
      base64?: string;
      mimetype?: string;
      filename?: string;
      caption?: string;
      mentions?: string[];
    },
  ): Promise<{ messageId: string }> {
    return this.withRetry(() =>
      this.client.messages.sendDocument(this.sessionName, { chatId, ...options }),
    );
  }

  /** Send an image via URL or base64 */
  async sendImage(
    chatId: string,
    options: {
      url?: string;
      base64?: string;
      mimetype?: string;
      caption?: string;
      mentions?: string[];
    },
  ): Promise<{ messageId: string }> {
    return this.withRetry(() =>
      this.client.messages.sendImage(this.sessionName, { chatId, ...options }),
    );
  }

  /** Access raw client for advanced operations (webhooks, groups, etc.) */
  getClient(): OpenWAClient {
    return this.client;
  }

  // ---- Internal ----

  /** Retry helper for transient errors (503, 429) */
  private async withRetry<T>(
    fn: () => Promise<T>,
    attempt = 0,
  ): Promise<T> {
    try {
      return await fn();
    } catch (error) {
      const isRetryable =
        error instanceof OpenWAServiceUnavailableError || // 503
        error instanceof OpenWARateLimitError; // 429

      if (!isRetryable || attempt >= this.maxRetries) {
        this.logger.error(
          `OpenWA call failed after ${attempt} retries`,
          error instanceof Error ? error.stack : error,
        );
        throw error;
      }

      const delay = this.baseRetryDelay * Math.pow(2, attempt);
      this.logger.warn(
        `OpenWA transient error (attempt ${attempt + 1}/${this.maxRetries}), retrying in ${delay}ms`,
        error instanceof Error ? error.message : String(error),
      );
      await new Promise((resolve) => setTimeout(resolve, delay));
      return this.withRetry(fn, attempt + 1);
    }
  }
}